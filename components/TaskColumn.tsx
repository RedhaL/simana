...
import { Dispatch, ICategory, IDate, ITask } from '../types';
...
const TodoColumn: FC<Props> = ({ element, size, taskDispatch, tasks }) => {
    const title = (element.hasOwnProperty("timestamp")) ?
        `${capitalizeFirstLetter(element.dayName)} ${element.day}/${element.monthName}` :
                ...
                >
                    {(providedDraggable: DraggableProvided, snapshotDraggable: DraggableStateSnapshot) => (
                        <div
                            ...
                            {...providedDraggable.dragHandleProps}
                        >
                            <TaskInput dispatch={taskDispatch} columnId={element._id} timestamp={element.timestamp}/>
                        </div>
                    )}
                </Draggable>
            </ColumnBG>
